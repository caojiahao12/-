...
void Dialog::readread()
{
    char data[16]={0};
    int temperature,humidity,luminance;

    if(mSerialPort.read(data,16))//mSerialPort->read(data,16)
    {
        if(data[0] == 'C' && data[1] == 'O'&& data[2] == 'M'
                && data[3] == 'M')
        {
            temperature = data[4];
            temperature = temperature<<8|data[5];
            temperature = temperature<<8|data[6];
            temperature = temperature<<8|data[7];

            humidity = data[8];
            humidity = humidity<<8|data[9];
            humidity = humidity<<8|data[10];
            humidity = humidity<<8|data[11];

            luminance = data[12];
            luminance = luminance<<8|data[13];
            luminance = luminance<<8|data[14];
            luminance = luminance<<8|data[15];

            if(temperature>0 )
            {
                ui->lineEdit_tem->setText(QString::number(temperature));
            }
            if(humidity>0)
            {
                ui->lineEdit_hum->setText(QString::number(humidity));
            }
            if(luminance>0)
            {
                ui->lineEdit_lum->setText(QString::number(luminance));
            }
        }
    }
}
